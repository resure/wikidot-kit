...
export default class WikidotKit {
  ...
  async fetchPageVotes(wikiUrl: string, pageName: string): Promise<WKUserVote[]> {
  ...
  async fetchPageRevisionsList(wikiUrl: string, pageName: string): Promise<WKPageRevisionMeta[]> {
      const pageUrl = `${wikiUrl}/${pageName}`;
      const pageID = await this.resolvePageId(pageUrl);
      if (!pageID) {
          throw new Error(`Page ${pageName} ID cannot be resolved`);
      ...
      const revisionRows = Array.from($('.page-history tr:not(:first-child)'));
      ...
      const revisions = revisionRows.map((row: CheerioElement) => {
          ...
          const date = new Date($(row).find('td:nth-child(6)').text().replace('\n\t\t', ''));
      ...
      const filteredRevisions = revisions.filter((revision) => revision !== null) as WKPageRevisionMeta[];

      return filteredRevisions;
  }

  async fetchPageRevisionContent(wikiUrl: string, revisionId: number): Promise<string> {
      const $ = await this.ajaxCall(wikiUrl, {
